...
const WaitingQueue = () => {
    ...
    return ( 
        <>
            ...
            />
            <Modal
                title= {statusUid === 'จองคิวสำเร็จ' ? 'จองคิวสำเร็จ' : statusUid === 'คิวเต็ม' ? 'คิวเต็ม' : 'ไม่สามารถจองคิวได้โปรดจองคิวใหม่อีกครั้ง'}
                visible={isNaN(statusUid)}
                footer={null}
                closable={false}
            >
                {statusUid === 'จองคิวสำเร็จ' ? (
                    <Button type="primary" style={{backgroundColor:"#f687b3 "}} onClick={() => router.push(`/student/${studentId}/home`)}>
                        กลับหน้าแรก (Back to homepage)
                    </Button>
                ) : (
                    <Button type="primary" style={{backgroundColor:"#f687b3 "}} onClick={() => router.push(`/student/${studentId}/appointment/${id}/0`)}>
